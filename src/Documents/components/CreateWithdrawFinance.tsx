...
import { getFinanceEmployeeQueryOptions } from "@/api/query";
...
const CreateWithdrawFinance = ({
  open,
  onOpenChange,
}: CreateWithdrawDialogProps) => {
  const [step, setStep] = useState(1);
  const { data: financeEmployeeList } = useQuery(
    getFinanceEmployeeQueryOptions()
  );
  ...
  const selectedEmployee = financeEmployeeList?.find(
    (emp) => emp.finance_person_code === selectedCode
  );
  const systemAmount = selectedEmployee?.current_total || 0;

  const difference = currentCash - systemAmount;
  const {
    mutate: createWithdraw,
    isPending,
  ...
  const getStatusText = () => {
    ...
    if (difference > 0)
      return `Cash amount is over by ${difference.toFixed(2)} LYD`;
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          ...
          {step === 1 ? (
            <Form {...form}>
              <form
                className="space-y-4 mt-4 w-[23.313rem]"
                onSubmit={(e) => {
                  e.preventDefault();
                  onNext();
                }}>
                    ...
                    <FormItem className="space-y-2">
                      <FormControl>
                        <Select
                          ...
                          <SelectContent>
                            {financeEmployeeList &&
                              financeEmployeeList.map((employee) => (
                                <SelectItem
                                  key={employee.finance_person_code}
                                  value={employee.finance_person_code!.toString()}>
                                  {employee.finance_person_name}
                                </SelectItem>
                              ))}
                          </SelectContent>
                        </Select>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-4">
                      <FormLabel className="font-semibold text-sm text-Primary-500">
                        Withdraw amount
                      </FormLabel>
                      <FormControl>
                        <Input
              ...
              <div className="space-y-2 *:h-10">
                <p className="text-sm font-semibold flex items-center text-Gray-900">
                  ...
                  <span className="font-semibold text-base text-Black">
                    &nbsp;{systemAmount.toFixed(2)}&nbsp;LYD
                  </span>
                </p>
                <p className="text-sm font-semibold flex items-center text-Gray-900">
                  Written withdraw amount :
                  <span className="font-semibold text-base text-Black ">
                    &nbsp;{currentCash.toFixed(2)}&nbsp;LYD
                  </span>
                </p>
              </div>
...
export default CreateWithdrawFinance;
