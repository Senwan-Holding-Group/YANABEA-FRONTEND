import { getTopItemMovementkQueryOptions } from "@/api/query";
import { useQuery } from "@tanstack/react-query";
import ItemStockSkeleton from "../../components/skeletons/ItemStockSkeleton";
import type { TopItemMovement } from "@/lib/types";
import { useAuth } from "@/api/Auth/useAuth";
type ItemStockProps = {
  className: string;
};
const ItemMovement = ({className}:ItemStockProps) => {
  const {user}=useAuth()
      const {
    data: topItemMovement,
    isFetching: isTopItemMovementFetching,
    isError: isTopItemMovementError,
  } = useQuery(getTopItemMovementkQueryOptions());
  return (
     <div className={`${className}  flex-1 overflow-y-scroll rounded-xl p-4 bg-Primary-25 shadow-CS`}>
          ...
          {isTopItemMovementFetching ? (
            <ItemStockSkeleton />
          ) : isTopItemMovementError ? (
            ...
            topItemMovement && (
              <div className={`grid ${user.user_type==="S"?" 3xl:grid-cols-2":"lg:grid-cols-2 xl:grid-cols-3"} gap-4`}>
                {topItemMovement.map((topItemStock: TopItemMovement) => (
